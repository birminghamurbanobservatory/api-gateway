...
router.get('/context/check.jsonld', asyncWrapper(async (req, res): Promise<any> => {
  const context = getCheckContext();
  return res.json(context);
}));

router.get('/context/user.jsonld', asyncWrapper(async (req, res): Promise<any> => {
